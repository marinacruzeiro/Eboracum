...
public abstract class BenchmarksGenerator {
	...
	protected double wurCover; // Eu adicionei
	...
	protected double sleepCost;
	protected double wakeUpSignalCost;
	protected double wusProcessingCost;
	...
	protected void generateModel(String simulationIdentification){
		...
		try {
			...
			//For WuR:
			changePropertyAttribute(doc, "WuRCover", Double.toString(this.wurCover));
			changePropertyAttribute(doc, "SleepCost", Double.toString(this.sleepCost));
			changePropertyAttribute(doc, "WakeUpSignalCost", Double.toString(this.wakeUpSignalCost));
			changePropertyAttribute(doc, "WuSProcessingCost", Double.toString(this.wusProcessingCost));
			...
			for (Map.Entry<String, Integer> entry : this.wirelessNodes.entrySet()) {
				for (int i=0; i<entry.getValue(); i++) {
					...
					//((Parameter)e.getAttribute("WuRCover")).setExpression(String.valueOf(this.wurCover));
					((Parameter)e.getAttribute("SleepCost")).setExpression(String.valueOf(this.sleepCost));
					((Parameter)e.getAttribute("WakeUpSignalCost")).setExpression(String.valueOf(this.wakeUpSignalCost));
	                                ((Parameter)e.getAttribute("WuSProcessingCost")).setExpression(String.valueOf(this.wusProcessingCost));
	...
	private void createDataReportFile(String simulationIdentification, int round) {
        ...
        try {
			...
			//eu adicionei a proxima linha:
			writer.write("Wake-up Radio Cover Radius;"+this.wurCover); writer.newLine();
